...
### HIGH
- **R-1 — `loadIssue` decided "fork exists" by repo NAME only.** ✅ **FIXED (PR #143, merged `a0e7f28`) — NEEDS HANDS-ON TESTING.** Now resolves the fork by its **parent** (`_userForkOf`) and clones it by full `owner/name`; `upstream` set to the canonical. Verified end-to-end in Slicer against a real name-collision fixture (cloned the suffixed fork `amm554/mdtest-collide-1` with canonical content, not the unrelated same-named repo). *Original:* owning an unrelated repo with the same name — or a stale/suffixed fork after a recreate — made it clone that instead of the real fork (silent wrong-repo; PR fails with "no shared history").

### MEDIUM
- **R-2 — in-org PR load was fork-assuming.** ✅ **FIXED (PR #143, merged `a0e7f28`) — NEEDS HANDS-ON TESTING.** `loadPR` now resolves the real head repo via `gh pr view --json headRepository,headRepositoryOwner,headRefName` and clones that; `_ensureUpstream` points `upstream` at the canonical explicitly (replacing the infer-from-origin `ensureUpstreamExists`). Verified end-to-end in Slicer. *Original:* `loadPR` built the head as `{author}/{name}` and upstream was inferred from origin, so in-org / suffixed-fork PRs cloned the wrong repo and reviewer actions 404'd.
- **R-3 — "Request PR review" can no-op on a just-created PR.** ⚪ **WON'T FIX — by design (lag self-resolves).** Within the RepoClerk journal's ~60s lag, `gh pr ready` may not have run yet; a refresh once the journal drains shows it ready. A second commit *in* that window surfaces a spurious "commit failed" (the push succeeded; GitHub **prevents** a true duplicate PR from the same head→base — verified, so the original "duplicate PR" claim was wrong). No data loss, no duplicate; real-time review only happens in workshops/tests where 60s clears it. Optional 2-line guard (treat "already exists" as success) noted, not taken.
- **R-4 — `_isArchivalRepo` transient gh error skipped contributor curation.** ✅ **FIXED (PR #145).** Defaults to True (curate) on error/empty + logs; a definitive User owner still returns False — a transient outage no longer yields a release with no contributor record.
- **R-5 — Release-tab open issue/PR counts were always zero.** ✅ **FIXED (PR #145).** `morphoRepos()` now carries `issues`/`pullRequests` totalCount + nodes from the journal; counts, announcement targets, and tooltip are accurate.
- **R-6 — Auto-assign workflow-scope check was one-shot.** ✅ **FIXED (PR #145).** Caches only a positive result; granting the scope takes effect on the next Create-tab entry without a reload.
- **R-7 — Search list-criterion exclusion was order-dependent.** ✅ **FIXED (PR #145).** Decided after scanning all values (`any(...)`); empty lists now respect the filter.
- **R-8 — Search tooltip assumed dict captions.** ✅ **FIXED (PR #145).** Normalizes list-shaped captions and guards a null value, so `.items()` can't crash.
- **R-9 — App enrich froze a heuristic member name on ORCID failure.** ✅ **FIXED (PR #145 extension side + `control_plane.py` App side, redeployed).** Emits `Family, Given` only when ORCID resolves; otherwise the onboarding name verbatim — never the heuristic reorder.
